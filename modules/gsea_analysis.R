...
gsea_analysis_server <- function(input, output, session, user_session, deg_results) {
  ...
  gsea_results <- eventReactive(input$run_gsea, {
    req(deg_results(), input$gmt_file, user_session$logged_in)
    ...
    if ("GseaVis" %in% loadedNamespaces()) {
      ...
      if (!is.null(genes_to_add) && length(genes_to_add) > 0 && !is.null(top_genes_data)) {
        ...
        tryCatch({
          ...
          # clusterProfiler的GSEA对象包含@geneList（ranked list）
          # 我们需要计算running enrichment score

          # 获取当前通路的基因集
          pathway_genes <- gsea_obj@result$geneID[selected]
          pathway_genes_list <- unlist(strsplit(pathway_genes, "/"))

          # 计算running enrichment score
          # 这是从clusterProfiler的GSEA实现中得出的方法
          ...
          running_score <- running_score / max(abs(running_score))  # 归一化
