...
tf_activity_server <- function(input, output, session, user_session, deg_results) {

  # 缓存 CollecTRI 网络
  collectri_net <- reactive({
    req(input$data_source, user_session$logged_in)
  ...
  tf_activity_results <- eventReactive(input$run_tf_activity, {
    req(deg_results(), collectri_net(), user_session$logged_in)
  ...
  selected_tf_targets <- reactive({
    req(tf_activity_results(), collectri_net(), deg_results(), user_session$logged_in)
