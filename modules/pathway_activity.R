...
pathway_activity_server <- function(input, output, session, user_session, deg_results, kegg_results) {
  ...
  pathway_activity_results <- eventReactive(input$run_pathway_activity, {
    req(deg_results(), kegg_results(), user_session$logged_in)
    ...
    pathway_net <- kegg_data %>%
      ...
      filter(!is.na(log2FC)) %>%  # 移除没有表达数据的基因
      group_by(pathway) %>%
      mutate(
        mor = ifelse(log2FC > 0, 1, -1)  # 基于差异表达计算 MOR
      ) %>%
      ungroup() %>%
      select(
        source = pathway,
