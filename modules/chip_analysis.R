...
#' 流式扫描文件，定位 Series Matrix 表起止行（不将整个文件读入内存）
#' @param file_path 文件路径
#' @return list(start_idx, end_idx, metadata_lines) 或 NULL 表示未找到
find_series_matrix_bounds <- function(file_path) {
  con <- file(file_path, "r", encoding = "UTF-8")
  on.exit(close(con), add = TRUE)
  chunk_size <- 50000L
  start_idx <- end_idx <- NA_integer_
  metadata_lines <- character(0)
  line_num <- 0L
  while (length(chunk <- readLines(con, n = chunk_size, warn = FALSE)) > 0) {
    n <- length(chunk)
    line_num <- line_num + n
    if (is.na(start_idx)) {
      idx <- which(chunk == "!series_matrix_table_begin")
      if (length(idx) > 0) {
        start_idx <- line_num - n + idx[1] + 1L   # 表头行（begin 下一行）
        metadata_lines <- c(metadata_lines, chunk[seq_len(idx[1] - 1L)])
      } else {
        metadata_lines <- c(metadata_lines, chunk)
      }
    }
    if (!is.na(start_idx) && is.na(end_idx)) {
      idx <- which(chunk == "!series_matrix_table_end")
      if (length(idx) > 0)
        end_idx <- line_num - n + idx[1] - 1L
    }
    if (!is.na(start_idx) && !is.na(end_idx)) break
  }
  if (is.na(start_idx))
    return(NULL)
  if (is.na(end_idx))
    end_idx <- line_num
  list(start_idx = start_idx, end_idx = end_idx, metadata_lines = metadata_lines)
}

#' 解析 GEO Series Matrix 文件（大文件优化：流式定位 + 按区段读取，可选 data.table::fread 加速）
#'
...
parse_geo_series_matrix <- function(file_path) {
  ...
  tryCatch({
    # 流式定位表起止行，避免整文件读入
    bounds <- find_series_matrix_bounds(file_path)
    if (is.null(bounds)) {
      return(list(
        success = FALSE,
    ...
    nrows <- end_idx - start_idx  # 数据行数（不含表头）

    cat(sprintf("✅ 找到数据区域: 第 %d - %d 行（共 %d 行数据）\n", start_idx, end_idx, nrows))

    # 使用 data.table::fread 加速（若可用），否则 read.table
    use_fread <- requireNamespace("data.table", quietly = TRUE)
    if (use_fread) {
      expr_matrix <- data.table::fread(
        file_path,
        skip = start_idx - 1L,
        nrows = nrows,
        header = TRUE,
      ...
      # 第一列作为行名
      rownames(expr_matrix) <- as.character(expr_matrix[[1]])
      expr_matrix <- expr_matrix[, -1L, drop = FALSE]
    } else {
    ...
    metadata <- extract_geo_metadata(bounds$metadata_lines)
...
#' 流式扫描 SOFT 文件，定位平台表起止行
#' 只把「在表开始之后」出现的 ^ 视为下一节，避免把 ^DATABASE/^SERIES/^PLATFORM 当结束
#' @param file_path 文件路径
#' @return list(start_idx, end_idx) 或 NULL
find_soft_platform_bounds <- function(file_path) {
  con <- file(file_path, "r", encoding = "UTF-8")
  on.exit(close(con), add = TRUE)
  chunk_size <- 50000L
  start_idx <- end_idx <- NA_integer_
  ...
  while (length(chunk <- readLines(con, n = chunk_size, warn = FALSE)) > 0) {
    ...
    line_num <- line_num + n
    if (is.na(start_idx)) {
      idx <- which(chunk == "!platform_table_begin")
      if (length(idx) > 0)
        start_idx <- line_num - n + idx[1] + 1L
    }
    if (!is.na(start_idx) && is.na(end_idx)) {
      idx <- which(grepl("^\\^", chunk))
      # 只认表开始之后出现的 ^（下一节），否则会误把 ^DATABASE/^SERIES/^PLATFORM 当结束
      if (length(idx) > 0) {
        first_after_start <- which((line_num - n + seq_len(n)) > start_idx)[1]
        if (!is.na(first_after_start)) {
          idx_after <- idx[idx >= first_after_start]
          if (length(idx_after) > 0)
            end_idx <- line_num - n + idx_after[1] - 1L
        }
      }
    }
    if (!is.na(start_idx) && !is.na(end_idx)) break
  ...
  if (is.na(end_idx)) end_idx <- line_num
  list(start_idx = start_idx, end_idx = end_idx)
}

#' 解析 SOFT 平台注释文件（大文件优化：流式定位 + 按区段读取，可选 fread 加速）
#'
#' @param file_path SOFT 文件路径
#' @param separator 字段分隔符（默认为 Tab，传给 read.table/fread）
...
parse_platform_annotation <- function(file_path, separator = "\t") {
  ...
  tryCatch({
    bounds <- find_soft_platform_bounds(file_path)
    if (is.null(bounds)) {
      return(list(
        success = FALSE,
    ...
    cat(sprintf("✅ 找到平台表: 第 %d - %d 行（共 %d 行数据）\n", start_idx, end_idx, nrows))

    use_fread <- requireNamespace("data.table", quietly = TRUE)
    if (use_fread) {
      raw_table <- data.table::fread(
        file_path,
        skip = start_idx - 1L,
        nrows = nrows,
        header = TRUE,
          ...
          fluidRow(
            column(6,
              ...
              fileInput("chip_series_matrix",
                       ...
                       placeholder = "选择文件..."),
              helpText("GEO 数据库下载的 Series Matrix 文件。大文件采用流式解析，请等待提示完成。")
            ),
            column(6,
              fileInput("chip_soft_platform",
                       ...
                       placeholder = "选择文件..."),
              helpText("用于探针注释的 GPL 平台文件。大文件采用流式解析，请等待提示完成。")
            )
          ),
...
#' @param user_session 用户会话信息
#' @param deg_results 差异分析结果容器（用于更新）
chip_analysis_server <- function(input, output, session, user_session, deg_results) {
  cat("✅ 芯片分析模块已启动\n")
  ...
  # 存储芯片数据（用轻量标志位供进度条使用，避免依赖大对象导致卡顿）
  chip_data <- reactiveValues(
    ...
    has_series_matrix = FALSE,
    has_merged_matrix = FALSE
  )
  ...
  observeEvent(input$chip_series_matrix, {
    ...
    # 大文件解析时保持提示直到完成（避免用户以为卡死）
    msg_id <- showNotification(
      "正在解析 GEO Series Matrix（大文件采用流式读取，请稍候）...",
      type = "message",
      duration = NULL
    )
    ...
    chip_data$has_series_matrix <- TRUE
  ...
  observeEvent(input$chip_soft_platform, {
    ...
    msg_id <- showNotification(
      "正在解析 SOFT 平台注释文件（大文件采用流式读取，请稍候）...",
      type = "message",
      duration = NULL
    )
    on.exit(removeNotification(msg_id), add = TRUE)
  ...
  # 列选择输入防抖（减少下拉框切换时的频繁重绘，缓解卡顿）
  chip_selection_inputs <- reactive({
    list(
      id_col = input$chip_soft_id_col %||% "",
      gene_col = input$chip_soft_gene_col %||% "",
      ent_col = input$chip_soft_entrez_col %||% ""
    )
  })
  chip_selection_inputs_d <- debounce(chip_selection_inputs, 400)
  ...
  # 🆕 状态提示单独渲染（带步骤进度；用防抖+轻量标志位减轻卡顿）
  output$chip_selection_status <- renderUI({
    sel <- chip_selection_inputs_d()
    id_col <- sel$id_col
    gene_col <- sel$gene_col
    entrez_col <- sel$ent_col

    step1_ok <- nzchar(id_col)
    step2_ok <- nzchar(gene_col)
    step3_ok <- isTRUE(chip_data$has_series_matrix)
    step4_ok <- isTRUE(chip_data$has_merged_matrix)
    n_done <- sum(step1_ok, step2_ok, step3_ok, step4_ok)
    ...
    if (!is.null(chip_data$soft_platform)) {
      ...
      if (length(found_cols) > 0) {
        possible_entrez_hint <- sprintf("💡 检测到可能的EntrezID列: %s", paste(found_cols, collapse = ", "))
      }
    }

    # 步骤进度条（始终显示，让用户知道当前进度）
    progress_box <- div(
      style = "background: #e7f1ff; padding: 12px 15px; border-radius: 8px; margin-top: 15px; border-left: 4px solid #2196F3;",
      h6("📌 探针注释进度", style = "color: #1565C0; margin-top: 0; margin-bottom: 10px; font-weight: bold;"),
      tags$ul(
        style = "margin: 0; padding-left: 20px; font-size: 13px;",
        tags$li(if (step1_ok) "✅ 步骤 1：已选择 ID 列" else "⬜ 步骤 1：请选择 ID 列", style = if (step1_ok) "color: #2e7d32;" else "color: #666;"),
        tags$li(if (step2_ok) "✅ 步骤 2：已选择基因列" else "⬜ 步骤 2：请选择基因列", style = if (step2_ok) "color: #2e7d32;" else "color: #666;"),
        tags$li(if (step3_ok) "✅ 步骤 3：已上传 Series Matrix" else "⬜ 步骤 3：请上传 Series Matrix 文件", style = if (step3_ok) "color: #2e7d32;" else "color: #666;"),
        tags$li(if (step4_ok) "✅ 步骤 4：已应用配置并生成矩阵" else "⬜ 步骤 4：请点击「应用配置并生成最终矩阵」", style = if (step4_ok) "color: #2e7d32;" else "color: #666;")
      ),
      p(
        style = "margin: 8px 0 0 0; font-size: 12px; font-weight: bold; color: #1565C0;",
        sprintf("当前进度：%d / 4 步已完成", n_done)
      )
    )
    ...
    if (!step1_ok || !step2_ok) {
      return(tagList(
        ...
        div(
          ...
          p("请先完成步骤 1 和 2：选择 ID 列与基因列。", style = "font-size: 12px; margin: 0;"),
          if (nchar(possible_entrez_hint) > 0) p(possible_entrez_hint, style = "font-size: 11px; color: #FF5722; margin: 5px 0 0 0;")
        )
      ))
    }
    ...
    if (!nzchar(entrez_col) && nchar(possible_entrez_hint) > 0) {
      status_color <- "#fff3cd"
      status_text <- "列选择已完成，建议选择 EntrezID 列（可选）"
    ...
    tagList(
      progress_box,
      div(
        style = sprintf("background: %s; padding: 10px; border-radius: 5px; margin-top: 10px;", status_color),
        h6(status_text, style = sprintf("color: %s; margin-top: 0;", status_text_color)),
        p(sprintf("ID列: %s | 基因列: %s", id_col, gene_col), style = "font-size: 12px; font-weight: bold;"),
        if (nzchar(entrez_col) && entrez_col != "自动检测") {
          p(sprintf("EntrezID列: %s", entrez_col), style = "font-size: 12px; color: #FF5722; font-weight: bold;")
        },
        if (!step4_ok) {
          p("请完成步骤 3、4：上传 Series Matrix 后点击「✅ 应用配置并生成最终矩阵」。", style = "font-size: 12px;")
        },
        if (nchar(possible_entrez_hint) > 0 && !nzchar(entrez_col)) {
  ...
  # 🆕 监听列选择（保存选择；列变更时标记“未应用”，进度步4 会显示未完成）
  observe({
    req(input$chip_soft_id_col, input$chip_soft_gene_col)
    req(nzchar(input$chip_soft_id_col), nzchar(input$chip_soft_gene_col))

    new_id   <- input$chip_soft_id_col
    new_gene <- input$chip_soft_gene_col
    old_id   <- chip_data$selected_id_col
    old_gene <- chip_data$selected_gene_col

    chip_data$selected_id_col <- new_id
    chip_data$selected_gene_col <- new_gene

    if (identical(new_id, old_id) && identical(new_gene, old_gene)) return(invisible())
    if (isTRUE(chip_data$has_merged_matrix)) {
      chip_data$has_merged_matrix <- FALSE
    }
  })
  ...
  observeEvent(input$chip_apply_merge, {
    ...
    # 用向量查找替代 merge，避免 200MB+ 大对象导致 cannot allocate vector
    probe_ids <- as.character(series_df$ProbeID)
    lookup_gene <- setNames(as.character(soft_df$GeneSymbol), as.character(soft_df$ID))
    series_df$GeneSymbol <- lookup_gene[probe_ids]
    has_entrez <- "EntrezID" %in% colnames(soft_df)
    if (has_entrez) {
      lookup_entrez <- setNames(soft_df$EntrezID, as.character(soft_df$ID))
      series_df$EntrezID <- lookup_entrez[probe_ids]
    }
    sample_cols <- setdiff(colnames(series_df), c("ProbeID", "GeneSymbol", "EntrezID"))
    merged_df <- series_df[, c("ProbeID", "GeneSymbol", if (has_entrez) "EntrezID", sample_cols)]
    ...
    chip_data$has_merged_matrix <- TRUE
