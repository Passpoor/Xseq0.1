...
# 🔧 安全的索引赋值辅助函数
# 避免索引不匹配导致的"替换数据里有0行"错误
safe_assign <- function(data, indices, values, colname) {
  # 检查索引是否有效
  if (length(indices) == 0) {
    cat(sprintf("⚠️  警告：试图更新列 '%s'，但索引为空\n", colname))
    return(data)
  }

  # 检查索引是否超出范围
  if (any(indices > nrow(data)) || any(indices < 1)) {
    cat(sprintf("⚠️  警告：试图更新列 '%s'，但索引超出范围\n", colname))
    return(data)
  }

  # 检查值和索引长度是否匹配
  if (length(indices) != length(values)) {
    cat(sprintf("⚠️  警告：试图更新列 '%s'，但索引和值长度不匹配 (%d vs %d)\n",
                colname, length(indices), length(values)))
    return(data)
  }

  # 安全赋值
  tryCatch({
    data[indices, colname] <- values
    return(data)
  }, error = function(e) {
    cat(sprintf("⚠️  警告：更新列 '%s' 失败: %s\n", colname, e$message))
    return(data)
  })
}

differential_analysis_server <- function(input, output, session, user_session) {

  # 获取数据输入模块的函数
  data_input <- data_input_server(input, output, session, user_session)
  ...
  deg_results <- eventReactive(input$analyze, {
    ...
    if (is.null(df) || !is.data.frame(df)) {
      showNotification("数据格式错误：不是有效的数据框", type = "error")
      return(NULL)
    }

    if (nrow(df) == 0) {
      showNotification("数据为空：没有基因数据", type = "error")
      return(NULL)
    }

    if (ncol(df) < 2) {
      showNotification("数据列数不足：至少需要2列（基因ID + 表达数据）", type = "error")
      return(NULL)
    }
    ...
    # 检查所选样本是否在数据中
    all_samples <- c(ctrl, trt)
    missing_samples <- setdiff(all_samples, colnames(df))
    if (length(missing_samples) > 0) {
      showNotification(sprintf("选择的样本不存在于数据中: %s", paste(missing_samples, collapse=", ")), type = "error")
      return(NULL)
    }
    ...
    tryCatch({
      ...
      # 添加差异状态 - 根据用户选择的p值类型
      pval_col <- input$pval_type  # "padj" 或 "pvalue"
      res$Status <- ifelse(
        res[[pval_col]] < input$pval_cutoff & abs(res$log2FoldChange) > input$log2fc_cutoff,
      ...
      if (!is.null(anno)) {
        ...
        # 清理anno中的列名
        # 🔧 修复：直接使用注释数据库返回的SYMBOL，不需要再次格式化
        # 注释数据库 (org.Mm.eg.db / org.Hs.eg.db) 已经返回了正确格式的基因符号
        anno_clean <- anno
        ...
        if ("ENSEMBL" %in% colnames(anno_clean)) {
          ...
          if (any(is_ensembl_id)) {
            ...
            # 🔧 修复：去除Ensembl ID的版本号后再匹配
            # 因为注释数据库返回的ENSEMBL列不包含版本号
            ensembl_ids_no_version <- gsub("\\..*", "", ensembl_ids)

            # 通过ENSEMBL列匹配（使用去除版本号后的ID）
            ensembl_match_idx <- match(ensembl_ids_no_version, anno_clean$ENSEMBL)
            matched_ensembl <- !is.na(ensembl_match_idx)
            n_matched <- sum(matched_ensembl)

            cat("通过ENSEMBL列匹配成功:", n_matched, "/", n_ensembl, "个基因\n")

            if (any(matched_ensembl) && sum(matched_ensembl) > 0) {
              ensembl_indices <- which(is_ensembl_id)
              if (length(ensembl_indices) >= sum(matched_ensembl)) {
                indices_to_update <- ensembl_indices[matched_ensembl]
                if (length(indices_to_update) > 0) {
                  res <- safe_assign(res, indices_to_update,
                                   anno_clean$SYMBOL[ensembl_match_idx[matched_ensembl]], "SYMBOL")
                  res <- safe_assign(res, indices_to_update,
                                   anno_clean$ENTREZID[ensembl_match_idx[matched_ensembl]], "ENTREZID")
                }
              }
            }
            ...
            if (n_matched < n_ensembl) {
              unmatched_ids <- ensembl_ids[!matched_ensembl]
        ...
        if ("SYMBOL" %in% colnames(anno_clean)) {
          ...
          if (any(unmatched)) {
            ...
            if (any(matched_genes) && sum(matched_genes) > 0) {
              unmatched_indices <- which(unmatched)
              if (length(unmatched_indices) >= sum(matched_genes)) {
                indices_to_update <- unmatched_indices[matched_genes]
                if (length(indices_to_update) > 0) {
                  res <- safe_assign(res, indices_to_update,
                                   anno_clean$SYMBOL[match_idx[matched_genes]], "SYMBOL")
                  res <- safe_assign(res, indices_to_update,
                                   anno_clean$ENTREZID[match_idx[matched_genes]], "ENTREZID")
          ...
          if (any(still_unmatched)) {
            ...
            if (any(matched_entrez) && sum(matched_entrez) > 0) {
              unmatched_indices <- which(still_unmatched)
              if (length(unmatched_indices) >= sum(matched_entrez)) {
                indices_to_update <- unmatched_indices[matched_entrez]
                if (length(indices_to_update) > 0) {
                  res <- safe_assign(res, indices_to_update,
                                   anno_clean$SYMBOL[entrez_match_idx[matched_entrez]], "SYMBOL")
                  res <- safe_assign(res, indices_to_update,
                                   anno_clean$ENTREZID[entrez_match_idx[matched_entrez]], "ENTREZID")
      ...
      # 🔧 添加注释状态列
      # 标记哪些基因成功注释（有ENTREZID且SYMBOL不是Ensembl ID格式）
      res$is_annotated <- !is.na(res$ENTREZID)

      n_annotated <- sum(res$is_annotated, na.rm = TRUE)
      n_not_annotated <- sum(!res$is_annotated, na.rm = TRUE)

      cat(sprintf("📊 注释统计: %d 个基因成功注释, %d 个基因未注释\n", n_annotated, n_not_annotated))

      if (n_not_annotated > 0) {
        cat(sprintf("⚠️  警告: %d 个基因未成功注释（可能是注释数据库版本过旧）\n", n_not_annotated))
        unannotated_samples <- head(res$SYMBOL[!res$is_annotated], 5)
        cat(sprintf("未注释示例: %s\n", paste(unannotated_samples, collapse = ", ")))
        cat("这些基因将在富集分析时被自动过滤\n")
      }

      # 过滤假基因
      res <- data_input$filter_pseudo_genes(res)

      # 🔧 新增：过滤未注释的基因（但不包括 Ensembl ID，因为它们可能有 ENTREZID）
      if ("is_annotated" %in% colnames(res)) {
        n_before <- nrow(res)
        # 只保留成功注释的基因或有 ENTREZID 的基因
        res <- res[res$is_annotated, ]
        n_removed <- n_before - nrow(res)
        if (n_removed > 0) {
          cat(sprintf("🧹 过滤了 %d 个未注释的基因\n", n_removed))
          showNotification(
            sprintf("自动过滤了 %d 个未注释的基因", n_removed),
      ...
      error_msg <- if (!is.null(e$message)) e$message else as.character(e)
      showNotification(paste("差异分析失败:", error_msg), type = "error")
      cat("差异分析错误:", error_msg, "\n")
      return(NULL)
    })
  })
  ...
  deg_results_from_file <- eventReactive(input$load_deg, {
    ...
    if (!is.null(anno)) {
      ...
      if ("ENSEMBL" %in% colnames(anno_clean)) {
        ...
        if (any(is_ensembl_id)) {
          # 🔧 修复：去除Ensembl ID的版本号后再匹配
          # 因为注释数据库返回的ENSEMBL列不包含版本号
          ensembl_ids_no_version <- gsub("\\..*", "", clean_geneid[is_ensembl_id])
    ...
    # 🔧 添加注释状态列
    # 标记哪些基因成功注释（有ENTREZID且SYMBOL不是Ensembl ID格式）
    res$is_annotated <- !is.na(res$ENTREZID) & !grepl("^ENS", res$SYMBOL, ignore.case = TRUE)
    ...
    # 🔧 新增：过滤未注释的 Ensembl ID 格式基因
    if ("is_annotated" %in% colnames(res)) {
      n_before <- nrow(res)
      # 只保留成功注释的基因（过滤掉 is_annotated = FALSE 的）
      res <- res[res$is_annotated, ]
      n_removed <- n_before - nrow(res)
      if (n_removed > 0) {
        cat(sprintf("🧹 过滤了 %d 个未注释的基因（包括 Ensembl ID 格式）\n", n_removed))
        showNotification(
          sprintf("自动过滤了 %d 个未注释的基因", n_removed),
          type = "message", duration = 5
        )
  ...
  chip_results_from_file <- eventReactive(input$load_chip, {
    req(data_input$chip_file_data(), user_session$logged_in)
    ...
    if (requireNamespace("qvalue", quietly = TRUE)) {
      qvalue_result <- tryCatch({
        qobj <- qvalue::qvalue(p = res$pvalue)
        list(success = TRUE, qvalues = qobj$qvalues, pi0 = qobj$pi0)
      }, error = function(e) {
        cat("⚠️  Q-value计算失败，使用padj作为替代:", e$message, "\n")
        list(success = FALSE, error = TRUE)
      })

      if (qvalue_result$success) {
        res$qvalue <- qvalue_result$qvalues
        cat(sprintf("✅ Q-value计算成功，π0估计值: %.3f\n", qvalue_result$pi0))
      } else {
        if ("padj" %in% colnames(res)) {
          res$qvalue <- res$padj
        } else {
          cat("⚠️  padj也不存在，使用pvalue作为qvalue\n")
          res$qvalue <- res$pvalue
        }
      }
    } else {
      cat("⚠️  qvalue包未安装，使用padj作为qvalue\n")
      if ("padj" %in% colnames(res)) {
        res$qvalue <- res$padj
      } else {
        cat("⚠️  padj不存在，使用pvalue作为qvalue\n")
        res$qvalue <- res$pvalue
      }
    }
    ...
    if (is.null(data) || nrow(data) == 0) {
      return(
        tags$div(
          ...
          p("可能的原因："),
          tags$ul(
            tags$li("所有基因在过滤过程中被移除（假基因、未注释基因等）"),
            tags$li("差异分析没有找到显著的差异表达基因"),
            tags$li("输入数据格式不正确或数据质量问题")
          ),
          p("建议：请检查控制台日志，了解具体的过滤和分析过程。")
        )
      )
    }
  ...
  # --- 基因助手查询 ---
  output$gene_assistant_output <- renderText({
    "在这里显示基因助手查询结果..."
  })
  ...
  observeEvent(input$ask_gene_assistant, {
    ...
    if (api_key == "") {
      showNotification("未配置DeepSeek API密钥，请在'账户 -> API配置'中设置", type = "error")
      return()
    }

    showNotification("正在咨询基因助手...", type = "message")

    tryCatch({
      # 构建基因助手提示词
      api_prompt <- paste(
        "你是一个专业的生物信息学专家，专门帮助研究人员理解基因功能和生物学过程。",
        "请用中文回答用户的问题，提供专业、准确、详细的解释。",
        "如果用户询问具体的基因列表，请提供完整的基因符号和简要功能描述。",
        "如果用户询问基因功能，请提供详细的生物学解释。",
        "如果用户询问技术问题，请提供实用的建议。",
        "请确保回答内容专业、准确、易于理解。",
        "",
        "用户问题：",
      ...
      response <- httr::POST(
        ...
        body = jsonlite::toJSON(list(
          model = "deepseek-chat",
          messages = list(
            list(role = "user", content = api_prompt)
          ),
          max_tokens = 2000,
          temperature = 0.7,
          stream = FALSE
        ...
        encode = "json",
        httr::timeout(30)  # 30秒超时
      )

      # 检查响应
      if (httr::status_code(response) == 200) {
        content <- httr::content(response, "parsed")
        assistant_response <- content$choices[[1]]$message$content
        ...
        showNotification("基因助手已回答您的问题", type = "message")
      } else {
        output$gene_assistant_output <- renderText({
          paste("❌ API调用失败，状态码:", httr::status_code(response))
        })
        showNotification("基因助手查询失败", type = "error")
      }
  ...
  output$interactive_volcano <- renderPlotly({
    ...
    if (is.null(res) || !is.data.frame(res) || nrow(res) == 0) {
      showNotification("无法绘制火山图：差异分析结果为空", type = "error")
      return(NULL)
    }
